...
class Boat {
    ...
    loadModelWithLoader(modelPath) {
        ...
        console.log("Scene available for boat model:", !!this.scene);
        
        return new Promise((resolve, reject) => {
            try {
                // Add a timeout to prevent infinite loading
                const loadTimeout = setTimeout(() => {
                    console.error("Boat model loading timed out after 30 seconds");
                    
                    // Create a fallback simple mesh if loading fails
                ...
                loader.load(
                    ...
                    (gltf) => {
                        ...
                        console.log("Mesh assigned:", !!this.mesh);
                        
                        if (!this.mesh) {
                            console.error("Failed to assign mesh from GLTF scene");
                            // Create a fallback simple mesh if loading fails
                            this.createFallbackMesh();
                            resolve(this.mesh);
                            return;
                ...
                console.error("Exception in loadModelWithLoader:", e);
    ...
    update(environment, deltaTime) {
        ...
        console.log("Boat update - Environment:", { 
            wind: { direction: wind.direction, speed: wind.speed },
            waves: { direction: waves.direction, amplitude: waves.amplitude }
        });
        
        // Calculate forces
        const forces = this.calculateForces(wind, waves);
        console.log("Forces calculated:", forces.length(), "direction:", 
            forces.x.toFixed(2), forces.y.toFixed(2), forces.z.toFixed(2));
        
        // Apply acceleration based on forces
        const acceleration = forces.clone().divideScalar(this.mass);
        console.log("Acceleration:", acceleration.length(), 
        ...
        console.log("Velocity change:", 
            "from:", prevVelocity.length().toFixed(2), 
            "to:", this.velocity.length().toFixed(2));
        
        // Apply drag (simplified)
        this.velocity.multiplyScalar(1 - this.dragCoefficient * deltaTime);
        console.log("After drag - velocity:", this.velocity.length().toFixed(2));
        ...
        try {
            ...
            console.log("Position updated:", 
                "from:", prevPosition.toArray().map(v => v.toFixed(2)),
                "to:", this.position.toArray().map(v => v.toFixed(2)),
                "change:", movement.length().toFixed(2));
            
            // Update the mesh position if it exists
            if (this.mesh) {
                this.mesh.position.copy(this.position);
                console.log("Updated mesh position:", 
                    this.mesh.position.x.toFixed(2), 
                    this.mesh.position.y.toFixed(2), 
                    this.mesh.position.z.toFixed(2));
            } else {
                console.warn("Boat mesh is not defined during position update");
            }
        } catch (e) {
        ...
        console.log("Rotation angle:", rotationAngle.toFixed(4), "rad");
    ...
    createFallbackMesh() {
        ...
        // Create a simple boat shape using geometry
        const boatGeometry = new THREE.BoxGeometry(this.length, this.height, this.width);
        const boatMaterial = new THREE.MeshStandardMaterial({ color: 0x3366cc });
        const boatMesh = new THREE.Mesh(boatGeometry, boatMaterial);
        
        // Create a group to hold the boat and add details
        const boatGroup = new THREE.Group();
        boatGroup.add(boatMesh);
        
        // Add a cabin (smaller box on top)
        const cabinGeometry = new THREE.BoxGeometry(this.length * 0.4, this.height * 0.5, this.width * 0.7);
        const cabinMaterial = new THREE.MeshStandardMaterial({ color: 0xffffff });
        const cabinMesh = new THREE.Mesh(cabinGeometry, cabinMaterial);
        cabinMesh.position.set(0, this.height * 0.75, 0);
        boatGroup.add(cabinMesh);
        
        // Set the mesh to our fallback
        this.mesh = boatGroup;
        ...
        if (this.scene) {
            this.scene.add(this.mesh);
            console.log("Fallback boat added to scene successfully");
        } else {
            console.error("Scene not available, cannot add fallback boat mesh");
        }
    }
}
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("08ac677be196f24cb596")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.713212f0eff0a94e5366.hot-update.js.map