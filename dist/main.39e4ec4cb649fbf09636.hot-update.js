...
class Water {
    ...
    createWaterMesh() {
        ...
        try {
            ...
            const gl = this.getGLContext();
            if (gl) {
                // Set a flag to check in update
                this.checkForShaderErrors = true;
            }
            
        } catch (error) {
            console.error("Failed to create shader-based water material:", error);
            this.createFallbackWaterMaterial(geometry);
        }
        
        // Add the water mesh to the scene
        if (this.scene && this.waterMesh) {
            this.scene.add(this.waterMesh);
        }
    }
    
    /**
     * Create a simple fallback water material when shaders fail
     * @param {THREE.BufferGeometry} geometry - The water geometry
     */
    createFallbackWaterMaterial(geometry) {
        console.log("Using basic water material instead of shader-based water");
        
        // Free any resources from attempted shader creation
        if (this.waterMaterial) {
            this.waterMaterial.dispose();
        }
        
        // Create a simple material for the water
        ...
        // Refine the geometry to have more vertices for wave animation
        if (geometry.attributes.position.count <= 4) {
            geometry = new THREE.PlaneGeometry(this.width, this.height, 64, 64);
            geometry.rotateX(-Math.PI / 2);
        }
        
        // Create the mesh with the fallback material
        this.waterMesh = new THREE.Mesh(geometry, this.waterMaterial);
        this.waterMesh.receiveShadow = true;
    ...
    getWaterVertexShader() {
        return `
            ...
            void main() {
                ...
                // Calculate wave height using a simple sine wave
                vec3 pos = position;
                float waveX = sin(position.x * 0.05 + time * 0.5) * 0.5;
                float waveZ = cos(position.z * 0.05 + time * 0.5) * 0.5;
                pos.y += waveX + waveZ;
                
                // Transform normal to account for waves
                vec3 norm = normal;
                norm.x = waveX * 0.5;
                norm.z = waveZ * 0.5;
                norm = normalize(norm);
    ...
    getWaterFragmentShader() {
        return `
            ...
            void main() {
                // Simple blue water with wave pattern
                vec3 color = waterColor;
                
                // Add wave pattern
                float pattern = sin(vUv.x * 20.0 + time) * 0.05 + 
                               cos(vUv.y * 20.0 + time * 0.8) * 0.05;
                color = color * (0.9 + pattern);
    ...
    update(deltaTime, camera) {
        try {
            // Check for shader errors on first update
            if (this.checkForShaderErrors) {
                this.checkForShaderErrors = false;
                
                // Test if the shader is working
                const gl = this.getGLContext();
                const program = this.waterMaterial.program;
                
                if (gl && program && !program.diagnostics) {
                    // Everything is fine
                } else if (gl && program && program.diagnostics && !program.diagnostics.runnable) {
                    // Shader didn't compile, switch to fallback
                    console.warn("Water shader compilation failed, using fallback");
                    ...
                    this.createFallbackWaterMaterial(this.waterMesh.geometry);
                    
                    // Add the new water mesh to the scene
                    if (this.scene) {
                        this.scene.add(this.waterMesh);
                    }
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("5bb032b9ce6eefa565d1")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.39e4ec4cb649fbf09636.hot-update.js.map