...
class Boat {
    ...
    update(environment, deltaTime) {
        ...
        // Special turning boost at very low speeds to improve responsiveness
        // This makes the boat more maneuverable when almost stopped
        if (this.speed < 1 && this.rudderAngle !== 0 && Math.abs(this.throttle) > 0.1) {
            // Add a small amount of turning based directly on throttle and rudder
            const turnBoost = this.rudderAngle * Math.abs(this.throttle) * 0.003;
            // Direction depends on forward/reverse
            const directionMultiplier = this.throttle < 0 ? -1 : 1;
            this.angularVelocity.y += turnBoost * directionMultiplier;
        }
    ...
    calculateForces(wind, waves) {
        ...
        if (this.rudderAngle !== 0) {
            ...
            // Turning force increases with speed but caps at medium speed for better control
            // Allow some turning even at very low speeds
            const minSpeedFactor = 0.5; // Minimum turning force even when nearly stopped
            const speedFactor = Math.max(minSpeedFactor, Math.min(this.speed / 5, 1.5));
            
            const forceMagnitude = this.rudderForceCoefficient * this.rudderAngle * speedFactor;
            
            rudderForce.copy(rudderDirection).multiplyScalar(forceMagnitude);
            
            // Apply angular velocity based on rudder angle and speed
            // Use rudder effect even at very low speeds for responsive turning
            let angularFactor = 0.015;
            
            // Stronger turning at low speeds for better handling
            if (this.speed < 2) angularFactor = 0.025;
            
            // Special turning boost when nearly stopped
            if (this.speed < 0.5) angularFactor = 0.05;
            
            // Direction is reversed when going backward
            const directionModifier = this.velocity.dot(boatDirection) < 0 ? -1 : 1;
            
            // Calculate the new angular velocity based on rudder angle
            // This is the key change to fix the "getting stuck" issue - maintain continuous turning
            const targetAngularVelocity = this.rudderAngle * Math.max(this.speed, 0.8) * angularFactor * directionModifier;
            
            // Add an acceleration component to ensure continuous turning
            // This prevents the turning from getting "stuck" after a second
            const angularAcceleration = (targetAngularVelocity - this.angularVelocity.y) * 2.0;
            this.angularVelocity.y += angularAcceleration;
        } else {
            // Gradually reduce angular velocity when rudder is centered
        ...
        if (this.velocity.lengthSq() > 0) {
            ...
            // Increased drag at higher speeds
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("da7f0400fe114a7abcd8")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.aba9413b5b15259a6622.hot-update.js.map