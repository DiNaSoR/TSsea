...
class Renderer {
    ...
    setupControls() {
        ...
        if (this.followBoat) {
            ...
            // Setup camera to be directly behind the boat
            // Get boat's orientation
            const boatDirection = new THREE.Vector3(0, 0, 1);
            boatDirection.applyQuaternion(this.followBoat.orientation);
            
            // Position camera behind the boat (opposite to boat's direction)
            this.camera.position.set(
                boatPosition.x - boatDirection.x * 15,
                boatPosition.y + 8,  // Above the boat
                boatPosition.z - boatDirection.z * 15
            );
    ...
    /**
     * Maintains a fixed camera distance from the boat when using orbit controls
     * Ensures camera target follows the boat properly, especially at high speeds
     */
    maintainFixedCameraDistance() {
        if (!this.followBoat || !this.followBoat.mesh || !this.controls) return;
        
        // Get boat's current position, velocity and speed
        const boatPosition = this.followBoat.position.clone();
        const boatVelocity = this.followBoat.velocity;
        const boatSpeed = this.followBoat.speed;
        
        // Update camera target to match boat position immediately
        this.controls.target.copy(boatPosition);
        
        // Calculate the current distance between camera and boat
        const cameraToBoatVector = new THREE.Vector3().subVectors(this.camera.position, boatPosition);
        const currentDistance = cameraToBoatVector.length();
        
        // Define an ideal distance from the boat (can adjust based on preferences)
        const idealDistance = Math.max(15, 15 + boatSpeed * 0.5);
        
        // Calculate the direction vector from boat to camera (normalized)
        const directionToCamera = cameraToBoatVector.clone().normalize();
        
        // Determine the new camera position to maintain ideal distance
        const newCameraPosition = new THREE.Vector3().addVectors(
            boatPosition,
            directionToCamera.multiplyScalar(idealDistance)
        );
        
        // If boat is moving fast, snap camera position immediately
        // Otherwise, smoothly interpolate
        const speedThreshold = 10; // Knots
        
        if (boatSpeed > speedThreshold) {
            // Snap camera position when boat moves fast
            this.camera.position.copy(newCameraPosition);
        } else {
            // Smooth interpolation for lower speeds
            // Use faster interpolation at higher speeds, slower at low speeds
            const smoothFactor = Math.max(0.1, Math.min(boatSpeed / speedThreshold, 0.5));
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("1fe2511925be01176a6d")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.3557681f8f604fe706ad.hot-update.js.map