...
class Boat {
    ...
    initMesh() {
        try {
            ...
            // Create the boat wake effect
            this.createBoatWake();
            
            // Create the engine exhaust effect
    ...
    loadModelWithLoader(modelPath) {
        // Check if GLTFLoader is available globally
        if (window.GLTFLoader) {
            const loader = new window.GLTFLoader();
            this.loadWithLoader(loader, modelPath);
        } else {
    ...
    /**
     * Helper method to load model with a loader instance
     */
    loadWithLoader(loader, modelPath) {
        loader.load(
            modelPath,
            (gltf) => {
    ...
    /**
     * Create a fallback boat model if the 3D model fails to load
     */
    createFallbackBoat() {
        // Create a simple motorboat shape using primitive geometries
        ...
        // Cabin
        const cabinGeometry = new THREE.BoxGeometry(this.length / 2.5, this.height / 2, this.width * 0.8);
        const cabinMaterial = new THREE.MeshPhongMaterial({ color: 0xffffff });
        const cabin = new THREE.Mesh(cabinGeometry, cabinMaterial);
        cabin.position.set(-this.length / 8, this.height / 4, 0);
        boatGroup.add(cabin);
        
        // Engine
        const engineGeometry = new THREE.BoxGeometry(this.length / 6, this.height / 3, this.width / 3);
        const engineMaterial = new THREE.MeshPhongMaterial({ color: 0x333333 });
        const engine = new THREE.Mesh(engineGeometry, engineMaterial);
        engine.position.set(-this.length / 2 + this.length / 12, 0, 0);
        boatGroup.add(engine);
        
        // Windshield
        const windshieldGeometry = new THREE.BoxGeometry(this.length / 10, this.height / 3, this.width * 0.7);
        ...
        windshield.position.set(-this.length / 6, this.height / 3, 0);
        boatGroup.add(windshield);
        
        // Position and add to scene
        boatGroup.position.copy(this.position);
        boatGroup.rotation.y = Math.PI; // Rotate 180 degrees to face forward
        ...
        if (this.scene) {
            this.scene.add(this.mesh);
            console.log("Fallback boat created and added to scene");
            ...
            console.error("Scene not available for fallback boat");
        }
    }
    ...
    /**
     * Add a label above the boat with the player's name
    ...
    addLabel(text) {
        ...
        try {
            ...
            this.labelMesh.position.y = 2; // Position above the boat
            ...
            console.log('Player label added successfully');
        } catch (error) {
            console.error('Error creating boat label:', error);
        }
    }
}
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("a9d5d961e71c0f91513f")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.5351ff63515f66a731a8.hot-update.js.map