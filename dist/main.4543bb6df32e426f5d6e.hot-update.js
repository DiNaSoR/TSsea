...
class Boat {
    ...
    calculateForces(wind, waves) {
        ...
        if (this.rudderAngle !== 0) {
            ...
            // Realistic turning physics: turning capability decreases as speed increases
            // At very low speeds, turning is limited due to lack of water flow over rudder
            // At medium speeds, turning is most effective
            // At high speeds, turning becomes more difficult due to inertia and hydrodynamics
            
            // Realistic turning curve (inverted bell curve)
            let turnEfficiency = 0;
            
            if (speedKnots < 1) {
                // Very slow - limited turning due to insufficient water flow over rudder
                turnEfficiency = speedKnots * 0.3;
            } else if (speedKnots < 5) {
                // Increasing effectiveness as speed builds
                turnEfficiency = 0.3 + (speedKnots - 1) * 0.15;
            } else if (speedKnots < 10) {
                // Peak turning efficiency at medium speeds (5-10 knots)
                turnEfficiency = 0.9;
            } else {
                // Decreasing effectiveness at higher speeds due to inertia and water resistance
                turnEfficiency = 0.9 - Math.min(0.5, (speedKnots - 10) * 0.05);
            }
            
            // Apply realistic force scaling based on turn efficiency
            ...
            // Apply angular velocity based on realistic physics
            // Using moment of inertia for proper rotational inertia
            
            // Moment of inertia increases with boat size (approximating boat as rectangular)
            // I = (1/12) * mass * (length² + width²)
            const momentOfInertia = (1/12) * this.mass * (this.length * this.length + this.width * this.width);
            
            // Direction is reversed when going backward
            const directionModifier = this.velocity.dot(boatDirection) < 0 ? -1 : 1;
            
            // Calculate torque (rotational force)
            const torque = this.rudderAngle * turnEfficiency * directionModifier;
            
            // Angular acceleration = torque / moment of inertia
            const angularAcceleration = torque / momentOfInertia * 0.02; // Scaling factor for game feel
            
            // Apply angular acceleration over time (much slower than previous implementation)
            // This creates the sensation of realistic inertia when turning
            this.angularVelocity.y += angularAcceleration;
            
            // Limit maximum angular velocity based on boat type
            const maxAngularVelocity = 0.5; // Radians per second - much slower than before
            ...
            // Gradually reduce angular velocity when rudder is centered (water resistance)
            // Realistic boats continue turning for a while after rudder is centered
            this.angularVelocity.y *= 0.97; // Very slow return to straight - simulates inertia
        }
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("20ddd37dc94360b15ac3")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.4543bb6df32e426f5d6e.hot-update.js.map