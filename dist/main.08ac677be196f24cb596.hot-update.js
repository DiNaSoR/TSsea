...
class Game {
    ...
    update(deltaTime) {
        ...
        // Log update cycle periodically (every 60 frames to reduce spam)
        if (Math.floor(Date.now() / 1000) % 3 === 0) {
            console.log("Game update - deltaTime:", deltaTime, "running:", this.state.isRunning);
            
            // Log boat state periodically
            if (this.localBoat) {
                console.log("Boat state:", {
                    position: this.localBoat.position.toArray(),
                    velocity: this.localBoat.velocity ? this.localBoat.velocity.toArray() : null,
                    rudderAngle: this.localBoat.rudderAngle,
                    throttle: this.localBoat.throttle,
                    hasMesh: !!this.localBoat.mesh
                });
                
                // Log controls state
                if (this.controls) {
                    console.log("Controls state:", {
                        rudderAngle: this.controls.rudderAngle,
        ...
        // Update controls and apply to the local boat
        if (this.controls && this.localBoat) {
            // Update controls first
            this.controls.update(scaledDeltaTime);
            
            // Apply controls to the local boat
            this.localBoat.rudderAngle = this.controls.rudderAngle;
            this.localBoat.throttle = this.controls.throttle;
            
            console.log("Applied controls to boat:", {
                rudderAngle: this.localBoat.rudderAngle,
                throttle: this.localBoat.throttle
            });
        }
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("e67517dafe85828a9f5b")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.08ac677be196f24cb596.hot-update.js.map