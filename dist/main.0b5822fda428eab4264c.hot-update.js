...
class Water {
    ...
    getWaterVertexShader() {
        return `
            ...
            const float FALLBACK_WAVE_SCALE = 5.0;
            const float FALLBACK_SPEED1 = 1.0;
            const float FALLBACK_SPEED2 = 0.7;
            ...
            float getWaveHeight(vec2 pos, float time) {
                ...
                // Main waves
                height += sin(pos.x * 0.02 + time) * 0.5;
                height += cos(pos.z * 0.03 + time * FALLBACK_SPEED2) * 0.3;
                
                // Details
                height += sin(pos.x * 0.06 + time * 1.3) * 0.2;
                height += cos(pos.z * 0.08 + time * 0.8) * 0.1;
                
                return height * 0.5;
            }
            
            // Fallback normal calculation
            vec3 getFallbackNormal(vec2 pos, float time, float delta) {
                float h = getWaveHeight(pos, time);
                float hL = getWaveHeight(vec2(pos.x - delta, pos.y), time);
                float hR = getWaveHeight(vec2(pos.x + delta, pos.y), time);
                float hD = getWaveHeight(vec2(pos.x, pos.y - delta), time);
                float hU = getWaveHeight(vec2(pos.x, pos.y + delta), time);
                
                // Compute normal from heights
                vec3 normal;
                normal.x = (hL - hR) / (2.0 * delta);
                normal.z = (hD - hU) / (2.0 * delta);
                normal.y = 1.0;
            ...
            void main() {
                ...
                // Sample water texture or use fallback
                vec4 waterInfo = texture2D(waterTexture, uv);
                
                if (waterInfo.r > -99999.0) { // Check if texture has valid data
                    // Use texture data
                    pos.y += waterInfo.r * FALLBACK_WAVE_SCALE;
                    norm = normalize(vec3(waterInfo.b, 1.0, waterInfo.a));
                } else {
                    // Use fallback wave function
                    pos.y += getWaveHeight(pos.xz, time) * FALLBACK_WAVE_SCALE;
                    norm = getFallbackNormal(pos.xz, time, 0.1);
                }
    ...
    getWaterFragmentShader() {
        return `
            ...
            void main() {
                ...
                float fresnel = 0.02 + 0.98 * pow(1.0 - max(0.0, dot(viewDirection, vNormal)), 5.0);
                
                // Calculate sun reflection (specular highlight)
                float sunReflection = pow(max(0.0, dot(reflect(-viewDirection, vNormal), normalize(sunDirection))), 100.0);
                
                // Final color
                vec3 color = waterColor + sunReflection * vec3(1.0, 1.0, 0.8) * 0.5;
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("bab1d237a510c19d3540")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.0b5822fda428eab4264c.hot-update.js.map